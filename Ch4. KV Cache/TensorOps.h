...
// Chapter 4: Tensor operations
//
// Everything up to the last function is the Chapter 1, 2 and 3 ops, COMPLETED.
...
//
// Your task in this file is the last function: MultiHeadAttentionWithKVCache

namespace basicllm {

// Chapter 1, 2 and 3 ops, completed. The conventions have not changed: axis 0 is
...
// Given ops continue below (the Chapter 3 answers)
...
inline int64_t ArgMax(const Tensor& logits, int64_t pos) {
    ...
    for (int64_t v = 1; v < vocabSize; ++v) {
        if (logits.At(v, pos) > bestVal) {
            bestVal = logits.At(v, pos);
            best = v;
        }
    }
    return best;
...
// End of the given ops. Your Chapter 4 task starts here

// MultiHeadAttentionWithKVCache: attention for ONE new token against everything
// seen so far, using the cache instead of recomputing past keys and values.
//
//   q:      {dim, 1}     the new token's query, already RoPE'd
//   k:      {kv_dim, 1}  the new token's key, already RoPE'd
//   v:      {kv_dim, 1}  the new token's value
//   kCache: {kv_dim, seq_len}  THIS layer's key cache; column p = position p
//   vCache: {kv_dim, seq_len}  THIS layer's value cache
//   pos:    the new token's position
//
//   Step 1: write the new token into the cache. For every feature c:
//             kCache[c, pos] = k[c, 0]     and the same for vCache/v
//           kCache came in as a zero-copy view of the full cache tensor, so
//           this writes the REAL cache; the caller sees it on the next call.
//
//   Step 2: per head, attend the one query against the cached history:
//             qHead = Slice(q, 0, h * headDim, headDim)              {headDim, 1}
//             kHead = Slice(Slice(kCache, 0, h * headDim, headDim),  first cut the head,
//                           1, 0, pos + 1)                           then cut positions [0, pos]
//             vHead = the same double Slice on vCache
//             oHead = Attention(qHead, kHead, vHead, causal = false) {headDim, 1}
//           and write oHead into the output at feature offset h * headDim.
//
//   Why causal = false: the cache only CONTAINS positions 0..pos. The future
//   is not in there to be masked; slicing [0, pos + 1) IS the causal mask.
//
//   Output is {dim, 1}. Assumes n_kv_heads == n_heads (true for stories15M);
//   grouped-query attention is a later refinement
inline Tensor MultiHeadAttentionWithKVCache(const Tensor& q, const Tensor& k, const Tensor& v,
                                            Tensor& kCache, Tensor& vCache,
                                            int64_t pos, int numHeads) {
    // TODO: implement (Chapter 4, Task 1)
    return {};
}
